...
export default function SkillGoals({ t, user }: Props) {
  const [stats, setStats] = useState<any>(null);
  useEffect(() => {
    if (user) getOverviewStats(user.id).then(setStats);
  }, [user]);
    ...
    <Card className="border-0 shadow-lg shadow-slate-200/50 h-full rounded-2xl">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-bold flex items-center gap-2 text-slate-800">
          <Target className="text-pink-500" /> {t("learning.skillGoals")}
        </CardTitle>
      </CardHeader>
            ...
            <div key={g.skill}>
              <div className="flex justify-between mb-2 text-sm font-medium">
                ...
                <span className="text-slate-500">
                  {g.current} / {g.target} pts
                </span>
              </div>
              <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-1000 ${g.color}`}
                  style={{ width: `${percent}%` }}
                />
              </div>
            </div>

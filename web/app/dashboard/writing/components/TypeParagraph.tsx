...
type TypeParagraphProps = {
    ...
    setSelectedTypeParagraph: (typeparagraph: { slug: string; name: string } | null) => void;
};
            ...
            <div className="grid grid-cols-4 gap-4 mt-4 min-h-44">
                {typeParagraphs.map(typeParagraph => {
                    return (
                        ...
                        >
                            <Card
                                onClick={() => setSelectedTypeParagraph({ slug: typeParagraph.slug, name: typeParagraph.name })}
                                key={typeParagraph.id}
                                className={`
                                    flex flex-col h-full justify-start items-center gap-4 px-4
                                transition-all duration-300 border-2
                                ${selectedTypeParagraph && selectedTypeParagraph.slug === typeParagraph.slug
                            ...
                            >
                                <div className="flex items-center justify-center bg-gray-200 rounded-full w-fit p-4">
                                    <Icon name={typeParagraph.icon.name} color={typeParagraph.icon.color} className="h-6 w-6" />
                                </div>
                                <div className="flex flex-col items-center gap-2">
                                    <h3 className="text-lg font-semibold">{typeParagraph.name}</h3>
                                    <p className="text-md text-gray-500 text-center">{typeParagraph.description}</p>
                                </div>
                            </Card>
                        </motion.div>
                    )
